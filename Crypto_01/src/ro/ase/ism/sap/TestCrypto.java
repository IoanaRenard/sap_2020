...
public class TestCrypto {
	...
	public static void main(String[] args) 
		...
		byte[] mdValue2 = HashFunction.getHash("message.txt", BCProvider, "MD5");
		
		Util.printHex(mdValue, "SHA-256");
		Util.printHex(mdValue2, "MD5");
		
		byte[] mdValue3 = HashFunction.getHash("This is a secret message".getBytes(), 
				BCProvider, "SHA-256");
		
		Util.printHex(mdValue3, "SHA-256");
		
		//MAC
		byte[] macValue = HashMACFunction.getHashMAC(
				"message.txt", "password".getBytes(), BCProvider, "HmacSHA1");
		Util.printHex(macValue, "HmacSHA1" );
		
		//PBKDF2
		PBKDF2Function pbk = 
				new PBKDF2Function(10000, 128, "PBKDF2WithHmacSHA1", BCProvider);
		...
		System.out.println("\nTime (ms) needed to hash the user password: " + 
				(tFinal - tStart));
